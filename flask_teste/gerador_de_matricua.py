...
def realizar_processo_matricula(nome, cpf, ano, nome_curso):
    curso_alvo = None

    #Procura o curso na lista global
    for c in cursos_disponiveis:
        if c.nome.lower() == nome_curso.lower():
            curso_alvo = c
            break

    if not curso_alvo.registrar_vaga():
        return {"sucesso": False, "mensagem": "Curso não encontrado"}
    
    if curso_alvo.registrar_vaga():
        num_matricula = gerar_matricula(curso_alvo.codigo, cpf, ano)

        return{
            "sucesso": True,
            "matricula": num_matricula,
            "nome": nome,
            "curso": curso_alvo.nome
        }
    else:
        return{"sucesso": False, "mensagem": "Infelizmente não ha vaga "}
